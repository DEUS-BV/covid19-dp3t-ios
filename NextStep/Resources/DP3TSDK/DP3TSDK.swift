...
class DP3TSDK {
        ...
        getATracingServiceClient(forceRefresh: true) { [weak self] result in
            ...
            case let .success(service):
                self?.synchronizer.sync(service: service) { [weak self] result in
                    ...
                    if case let .failure(something) = result {
                        switch something {
                        case .BluetoothTurnedOff:
                            print("BluetoothTurnedOff")
                        ...
                        case let .NetworkingError(error):
                            print(error ?? "nil error on network error")
                        case .PermissonError:
    ...
    private func getATracingServiceClient(forceRefresh _: Bool, callback: @escaping (Result<ExposeeServiceClient, DP3TTracingErrors>) -> Void) {
