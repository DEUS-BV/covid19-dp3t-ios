...
class ExposeeServiceClient {
    ...
    public init(descriptor _: TracingApplicationDescriptor, urlSession: URLSession = .shared, urlCache: URLCache = .shared) {
        let customDescriptor = TracingApplicationDescriptor(appId: "covid.deus.ai",
                                                            description: "This is a deus demo app for COVID-19 using the DP-3T SDK",
                                                            backendBaseUrl: URL(string: "https://covid.deus.ai/")!,
                                                            contact: "https://deus.ai/#contactus")
        descriptor = customDescriptor
        self.urlSession = urlSession
        self.urlCache = urlCache
        exposeeEndpoint = ExposeeEndpoint(baseURL: customDescriptor.backendBaseUrl)
        managingExposeeEndpoint = ManagingExposeeEndpoint(baseURL: customDescriptor.backendBaseUrl)
    }
